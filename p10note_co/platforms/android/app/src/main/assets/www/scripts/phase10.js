$(document).ready(function () {
    ...
    gameInfo.hasPhaseCard;
    gameInfo.hasDealer;
    gameInfo.hasPlayers;
    gameInfo.hasPlayerNames;
    ...
    router
        .on(function () {
            ...
            if (!devMode) {
                $("#game-control-wrapper").css("visibility", "hidden");
    ...
    router
        .on("playersettings", function () {
            if (getNumberOfPlayers() < 2) {
                console.log(
                    "Routed from 'playersettings' to 'gamesettings' because the number of required players is too small."
                );
                showMessage("Es müssen mindestens 2 Spieler teilnehmen");
                router.navigate("gamesettings");
            } else {
                ...
                playersettings +=
                    ...
                    <div>
                        <div class='player-name-input-text'>
                            <input type='text' class='player-name-input' id='player-name-` +
                    i +
                    `' placeholder='Spieler ` +
                    (i + 1) +
                    `'>
    ...
    function generateDealerInput() {
        ...
        for (var i = 0; i < numPlayers; i++) {
            ...
            dealerdata +=
                `
                <div class="dealer-choose" data-dealer-id=` +
                i +
                `>
    ...
    function checkGameInfo() {
        if (!gameInfo.hasDealer) {
            router.navigate("dealerselection");
            showMessage("Bitte wähle aus, wer als erstes die Karten gibt");
            return false;
        }
        if (!gameInfo.hasPhaseCard) {
            router.navigate("gamecardselection");
            showMessage("Bitte wähle ein Spiel aus");
            return false;
        }
    ...
    function generatePhaseCards() {
        ...
        phases[8] = new Array();
        phases[8]["name"] = "Total Insanity";
        phases[8]["phase"] = new Array();
        phases[8]["phase"][0] =
            "3er Folge in einer Farbe + 4er Folge in einer Farbe + Drilling aus unterschiedlichen Farben";
        phases[8]["phase"][1] = "Vierling + 6 Karten einer Farbe";
        phases[8]["phase"][2] = "2 Fünflinge";
        phases[8]["phase"][3] = "Vierling + Zwilling + 4er Folge aus geraden oder ungerade Zahlen (Bsp.: 2,4,6,8)";
        phases[8]["phase"][4] = "3 aufeinanderfolgende Drillinge (Bsp.: 2+2+2, 3+3+3, 4+4+4)";
        phases[8]["phase"][5] = "Fünfling + 5er Folge in einer Farbe";
        phases[8]["phase"][6] = "5 Zwillinge in gleicher Farbe";
        phases[8]["phase"][7] = "10er Folge aus maximal 2 Farben";
        phases[8]["phase"][8] = "Siebenling + Drilling";
        phases[8]["phase"][9] = "10 Karten einer Farbe";

        phases[9] = new Array();
        phases[9]["name"] = "Chill'n'Grill";
        phases[9]["phase"] = new Array();
        phases[9]["phase"][0] = "3er Folge + Vierling";
        phases[9]["phase"][1] = "4er Folge + Vierling";
        phases[9]["phase"][2] = "Fünfling";
        phases[9]["phase"][3] = "Fünfling + Zwilling";
        phases[9]["phase"][4] = "6er Folge + 2 Zwillinge";
        phases[9]["phase"][5] = "7er Folge + Zwilling";
        phases[9]["phase"][6] = "7 Karten einer Farbe";
        phases[9]["phase"][7] = "3er Folge einer Farbe + Drilling + Zwilling";
        phases[9]["phase"][8] = "4er Folge einer Farbe + Drilling";
        phases[9]["phase"][9] = "5er Folge einer Farbe + Vierling";

        phases[10] = new Array();
        phases[10]["name"] = "Endgame";
        phases[10]["phase"] = new Array();
        phases[10]["phase"][0] = "6er Folge in einer Farbe + Vierling aus 2 Farben";
        phases[10]["phase"][1] = "5 Zwillinge in jeweils gleicher Farbe";
        phases[10]["phase"][2] = "Zwei 5er Folgen in jeweils gleicher Farbe";
        phases[10]["phase"][3] = "Sechsling + 2 Zwillinge in gleicher Farbe";
        phases[10]["phase"][4] = "Vierling + 2 Drillinge aufeinanderfolgend (4+4+4+4, 5+5+5, 6+6+6)";
        phases[10]["phase"][5] = "2 Fünflinge";
        phases[10]["phase"][6] = "Sechsling + 4er Folge in einer Farbe";
        phases[10]["phase"][7] =
            "Fünfling + Zwilling + 3er Folge aus geraden oder ungerade Zahlen (Bsp.: 2,4,6) in einer Farbe";
        phases[10]["phase"][8] = "4er Folge in einer Farbe + 3er Folge in einer Farbe + Zwilling in einer Farbe";
        phases[10]["phase"][9] = "Achtling";

        var num_cards = phases.length;

        for (i = num_cards - 1; i >= 0; i--) {
            $("#gamecards-wrapper").prepend(
                '<div class="phase-card-choose" data-card-id="' +
                    i +
                    '"><div class="phase-name left">' +
                    phases[i]["name"] +
                    '</div><div class="show-card right">i</div><div class="clearfix"></div></div>'
            );
        }
    }
            ...
            for (i = 0; i < num_phases; i++) {
                ...
                $("#phase-card").append(
                    '<tr class="phase-line ' +
                        phaseDone +
                        phaseActive +
                        '"><td class="phase-line-num">' +
                        (i + 1) +
                        '.)</td><td class="phase-line-desc">' +
                        phases[cardId]["phase"][i] +
                        "</td></tr>"
                );
            }
    ...
    function createGameTable() {
        ...
        if (getNumberOfPlayers() > 1) {
            for (var i = 0; i < getNumberOfPlayers(); i++) {
                ...
                playerinfodata +=
                    `
                <tr class="table-row" data-player-id="` +
                    i +
                    `" id="playerinfo_header_` +
                    i +
                    `">
                    <td class="table-cell" id="player-name-` +
                    i +
                    `">` +
                    players[i]["name"] +
                    `</td>
                    <td class="table-cell player-phase center-align" id="player-phase-` +
                    i +
                    `"><span class="clickable-text">` +
                    players[i]["phase"] +
                    `</span></td>
                    <td class="table-cell center-align player-trys" id="player-last-points-` +
                    i +
                    `"><span class="` +
                    tryClass +
                    `">` +
                    players[i]["last_points"] +
                    `</span></td>
                    <td class="table-cell center-align" id="player-points-` +
                    i +
                    `">` +
                    players[i]["points"] +
                    `</td>
    ...
    router
        .on("playertrys/:id", function (params) {
    ...
    function generateUserRounds(user) {
        ...
        if (numRounds > 0) {
            for (round = 0; round < numRounds; round++) {
                ...
                createUrRow(
                    players[user]["rounds"][round]["points"],
                    players[user]["rounds"][round]["completed"],
                    trys
                );
    ...
    function createUrHeader(phase) {
        $("#ur-round-wrapper").append(
            `
        <tr class="table-row border-bottom ur-tbl-header" id="ur-table-header-` +
                phase +
                `">
            <td class="left-align">
                <div class="left"><i class="material-icons main-color-light">done_all</i></div>
                <div class="ur-tr-header left" id="ur-hdr-phase-` +
                phase +
                `">Phase 1</div>
                <div class="clearfix"></div>
            </td>
            <td class="center-align">
                <div class="ur-tr-header right" id="ur-hdr-try-` +
                phase +
                `">0</div>
            ...
            <td class="center-align">
                <div class="ur-tr-header right" id="ur-hdr-points-` +
                phase +
                `">0</div>
    ...
    function createUrRow(points, done, trys) {
        ...
        <tr class="ur-removeable-round">
            <td class="left-align try-cell">Versuch ` +
                trys +
                `</td>
            <td class="right-align ur-td-cell"><img src="` +
                image +
                `"/></td>
            <td class="right-align ur-td-cell">` +
                points +
                `</td>
        </tr>`
        );
        ...
        for (i = 0; i < num_players; i++) {
            ...
            $(input_wrapper).append(
                ...
                <tr class="table-row">
                    ...
                    player_name +
                    `</td>
                    <td class="left-align"><input id="player-points-input-` +
                    i +
                    `" tabindex="` +
                    (i + 1) +
                    `" type="number" class="finish-points-input" data-checkbox-id="` +
                    i +
                    `" value="` +
                    points +
                    `"  ` +
                    ...
                    <td class="center-align">                
                        <div class="checkbox">
                            <input type="checkbox" id="player-phase-finish-` +
                    i +
                    `" ` +
                    phase +
                    `>
                            <label for="player-phase-finish-` +
                    i +
                    `"></label>
                        </div>
                    </td>
    ...
    function save_round(round) {
        ...
        for (i = 0; i < num_players; i++) {
            ...
            console.info(
                "Data saved: " +
                    players[i]["rounds"][rnd_num]["points"] +
                    " Points on " +
                    players[i]["name"] +
                    " (Round " +
                    rnd_num +
